...
public class IndicatorCountryOHLC1 : Indicator
{
    ...
    protected override void OnInit()
    {
        ...
        this.mainHistoricalData = this.Symbol.GetHistory(Period.MIN1, this.startOfHistory);
    ...
    protected override void OnUpdate(UpdateArgs args)
    {
        ...
        if (this.PreviousDataOffset >= 0)
        {
            ...
            if (this.newyorkSession.ContainsDate(currentCloseTime))
            {
                if (this.londonRangeCache.Count > this.PreviousDataOffset)
                    range = this.londonRangeCache[this.PreviousDataOffset];
                else
                    return;
                currentSessionString = "New York";
                previousSessionString = "London";
            }
            else if (this.asiaSession.ContainsDate(currentCloseTime))
            {
                if (this.newyorkRangeCache.Count > this.PreviousDataOffset)
                    range = this.newyorkRangeCache[this.PreviousDataOffset];
                else
                    return;
                currentSessionString = "Asia";
                previousSessionString = "New York";
            }
            else if (this.londonSession.ContainsDate(currentCloseTime))
            {
                if (this.asiaRangeCache.Count > this.PreviousDataOffset)
                    range = this.asiaRangeCache[this.PreviousDataOffset];
                else
                    return;
                currentSessionString = "London";
    ...
    private bool compareTwoAlerts(string first, string second)
    {
